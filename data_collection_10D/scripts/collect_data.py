...
def collect_A1_data(budget_factor, dim = 5, time_run=False):
    ...
    logger = ioh.logger.Analyzer(
        triggers=[trigger],
        folder_name=f'../data/run_data_10D/A1_data_newInstances_10D_new_sp/A1_B{budget_factor}_{dim}D',
...
def collect_A2(budget_factor, dim, A2, algname, run_A2_from_scratch=False, time_run=False, find_best=False):
    ...
    logger = ioh.logger.Analyzer(
        triggers=[trigger],
        folder_name=f'../data/run_data_10D/A2_data_newInstances_10D_scratch/A2_{algname}_B{budget_factor}_{dim}D',
            ...
            for rep in range(20):
                ...
                if run_A2_from_scratch:
                    if algname not in ["Same", "Non-elitist"]:
                        ...
                        algorithm = A2(problem, verbose=False, seed=np.random.get_state())
                        # Run for 1000 evals
                        algorithm.set_params({'budget': 2000})
                        algorithm.set_hyperparams({})
                        def stopping_criteria():
                            return problem.state.evaluations >= 2000
                    ...
                    alg = From_CMA_To_CMA(budget_factor, dim, algname, total_budget_factor=400)
                    alg(problem, algname)
                else:
                    alg = Switched_From_CMA(budget_factor, dim, A2, total_budget_factor=400)
                    alg(problem, A2)
...
def collect_all(x = None):
    ...
    for A2, algname in zip([MLSL, DE, PSO, BFGS, None, None], ["MLSL", "DE", "PSO", "BFGS", "Same", "Non-elitist"]):
        collect_A2(budget_factor, dim, A2, algname, time_run=False, run_A2_from_scratch=True)
    # Only run BFGS
...
def get_combinations():
    budget_factors = [20*i for i in range(1, 10)] + [100*i for i in range(2, 21)]
    # budget_factors = [300]
    dim = 10
