...
def process_ioh_data(base_path):
    ...
    for budget_dir in os.listdir(base_path):
        ...
        for func_dir in os.listdir(budget_path):
            ...
            numeric_cols = ['evaluations', 'raw_y', 'rep', 'iid', 'x0', 'x1', 'x2', 'x3', 'x4', 'x5', 'x6', 'x7', 'x8', 'x9']
        ...
        if all_rows:
            ...
            # Reorder columns
            column_order = ['fid', 'iid', 'rep', 'evaluations', 'raw_y', 'true_y', 'x0', 'x1', 'x2', 'x3', 'x4', 'x5', 'x6', 'x7', 'x8', 'x9']
...
def extract_a2_precisions(base_dir, output_file="A2_precisions.csv",
                          algorithms=None, budgets=None, fids=range(1, 25),
                          max_evals=1000):
    """
    Extracts minimum precision values from IOHprofiler logs for switched algorithms at different budgets.
    Writes results sequentially to CSV via pandas to_csv(..., mode='a', header=False).
    """
    ...
    if budgets is None:
        budgets = [10*i for i in range(1, 10)] + [50*i for i in range(2, 21)]

    dim = 10
    lower_bound, upper_bound = -5, 5

    # Initialize the output CSV with header (overwrite if exists)
    cols = ["fid", "iid", "rep", "budget", "algorithm", "precision"]
    ...
    for algo in algorithms:
        for budget in budgets:
            folder_name = os.path.join(base_dir, f"A2_{algo}_B{budget}_10D")
            ...
            for fid in fids:
                ...
                for func_folder in func_folders:
                    ...
                    file_path = os.path.join(folder_name, func_folder, f"IOHprofiler_f{fid}_DIM10.dat")
                    ...
                    try:
                        ...
                        # Coerce types
                        df['evaluations'] = pd.to_numeric(df.get('evaluations'), errors='coerce')
                        df['raw_y'] = pd.to_numeric(df.get('raw_y'), errors='coerce')
                        df['rep'] = pd.to_numeric(df.get('rep'), errors='coerce', downcast='integer')
                        df['iid'] = pd.to_numeric(df.get('iid'), errors='coerce', downcast='integer')
                        df = df.dropna(subset=['evaluations', 'raw_y', 'rep', 'iid'])

                        # Ensure numeric for x-columns if present
                        for i in range(dim):
                            col = f'x{i}'
                    ...
                    # Work per (rep, iid)
                    for (rep, iid), group in df.groupby(['rep', 'iid'], as_index=False):
                        ...
                        unfiltered_precision = float(min_row_unfiltered['raw_y'])

                        # Filter to in-bounds rows
                        x_cols = [f'x{i}' for i in range(dim) if f'x{i}' in subset.columns]
                        if len(x_cols) != dim:
                            # If x columns are missing, we can't check bounds—skip bounds filtering
                            in_bounds = subset.copy()
                        else:
                            within = subset[x_cols].ge(lower_bound) & subset[x_cols].le(upper_bound)
                            in_bounds_mask = within.all(axis=1)
                            in_bounds = subset[in_bounds_mask]

                        if in_bounds.empty:
                            print(f"❌ No in-bounds minimum for (fid={fid}, iid={int(iid)}, rep={int(rep)})")
                            continue

                        min_row_filtered = in_bounds.loc[in_bounds['raw_y'].idxmin()]
                        filtered_precision = float(min_row_filtered['raw_y'])
                        ...
                        if filtered_precision != unfiltered_precision:
                            print(f"⚠️ Changed: (fid={fid}, iid={int(iid)}, rep={int(rep)}) "
    ...
    # Final reporting and return the full DataFrame (read from file we built)
    if out_of_bounds_cases:
        print("\nOut-of-bounds minima detected:")
        for c in out_of_bounds_cases:
            print(c)
    else:
        print("\nNo out-of-bounds minima found.")

        # Read sequential file, sort, and save final CSV
    final_df = pd.read_csv(output_file)
    final_df.sort_values(by=["fid", "iid", "rep", "budget"], inplace=True)
...
def add_algorithm_precisions(ela_dir, precision_csv, output_dir):
    ...
    for file in os.listdir(ela_dir):
        ...
        if budget not in [10, 30, 50, 70, 90, 110, 130, 150, 170, 190]: continue
...
if __name__ == "__main__":
    ...
    #     "../data/run_data_10D/A2_data_10D_new_sp",
    #     "../data/precision_files/A2_precisions_new_sp_10D.csv",
    #     max_evals = 2000
    # )
    ...
    # )
    # process_ioh_data("../data/run_data_10D/A1_data_10D")
    # extract_a2_precisions(
    #     "../data/run_data_10D/A2_data_10D",
    #     "../data/precision_files/A2_precisions_10D_only_BFGS.csv",
    #     budgets = [20*i for i in range(1, 10)] + [100*i for i in range(2, 21)],
    #     algorithms = ["BFGS"],
    #     max_evals = 2000
    # )
    # df1 = pd.read_csv("../data/precision_files/A2_precisions_newInstances_10D.csv")
    # df2 = pd.read_csv("../data/precision_files/A2_precisions_newInstances_10D_BFGS.csv")
    # df = (
    #     pd.concat([df1, df2], ignore_index=True)
    #     .sort_values(['fid', 'iid', 'rep', 'budget', 'algorithm'])
    #     .reset_index(drop=True)
    # )
    # df.to_csv("../data/precision_files/A2_precisions_newInstances_10D.csv", index=False)
    # process_ioh_data("../data/run_data_10D/A1_data_10D_new_sp")
    # add_algorithm_precisions(
    #     ela_dir="../data/ela_with_cma_std/A1_data_ela_cma_std_10D",
    #     precision_csv="../data/precision_files/A2_precisions_10D_all.csv",
    #     output_dir="../data/ela_with_algorithm_precisions/A1_data_ela_cma_std_with_precisions"
    # )
    # budgets = [10, 30, 50, 70, 90, 110, 130, 150, 170, 190]
    # for budget in budgets:
    #     normalize_test_ela(
    #         f"../data/ela_with_cma_std/A1_data_ela_cma_std_10D/A1_B{budget}_10D_ela_with_state.csv",
    #         f"../data/ela_with_cma_std/A1_data_ela_cma_std_10D_newInstances/A1_B{budget}_10D_ela_with_state.csv",
    #         f"../data/ela_normalized/A1_data_ela_cma_std_10D_newInstances/A1_B{budget}_10D_ela_with_state.csv",
    #     )

    extract_a2_precisions(
        "../data/run_data_10D/A2_data_newInstances_10D_scratch",
        "../data/precision_files/A2_precisions_newInstances_10D_scratch.csv",
        budgets = [20],
        max_evals = 2000
    )
    # normalize_and_log_precision_files(
    #     "../data/precision_files/A2_precisions_10D_all.csv",
    #     "../data/precision_files/A2_precisions_10D_all_log10.csv"
    # )
    # extract_a2_precisions("../data/run_data_10D/A2_data_newInstances_10D_new_sp",
    #                       "../data/precision_files/A2_precisions_newInstances_10D_new_sp.csv",
    #                       budgets=[10, 30, 50, 70, 90, 110, 130, 150, 170, 190],
    #                       max_evals=2000)