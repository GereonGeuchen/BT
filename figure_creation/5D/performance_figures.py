...
optimal_selection_sum = 1434.2095246840001
optimal_switching_first = 1622.849555104
optimal_switching_last = 1464.4690965342
identified_switch = 2139.0665358946
...
def plot_gap_closed_bars(
    ...
    title: str = "Performance of Dynamic Selector vs. Baselines, 5D",
    font_family: str = "Latin Modern Roman",
    width: int = 880,
    height: int = 480,
...
def plot_selector_dashboard(
    ...
    for fid, sub in df.groupby("fid", dropna=False):
        ...
        for col in methods:
            ...
            name  = "Dynamic selector" if col == "selector_precision" else "B80"
    ...
    gap_mid    = (top_bottom + mid_top) / 2.0 - 0.09   # slightly lower to avoid overlap

    # ---- Layout with three legends (no titles)
    fig.update_layout(
        title=dict(text="Selector Behaviour across Functions, 5D", x=0.5, xanchor="center",
        pad=dict(t=0, b=0)),
        width=width, height=height,
        font=dict(family=font_family, size=16, color="black"),
...
if __name__ == "__main__":
    ...
    df_algo = pd.read_csv("data/selector_old_tuning_200_200_with_algos.csv")

    # _ = plot_selector_dashboard(df, df_algo, save_pdf="selector_dashboard_5D.pdf")

    plot_selector_dashboard(df, df_algo, save_pdf="selector_dashboard_5D_test.pdf", height=1400)
    # _,res = plot_gap_closed_bars(df, budgets=[80, 150, 8, 800], save_pdf="selector_bars_ablations.pdf")
    # print(res)