...
bfgs_sum = 271680256933.25082
de_sum   = 72558725.41275702
pso_sum  = 264483.22414435766
mlsl_sum = 616758.8854799567
same_sum = 50414.5505510575
nonelit_sum = 11016.2300601413
sbs_sum = 6755.964497405699

optimal_selection_sum = 3451.644919809400
optimal_switching_first = 2625.574349896601
optimal_switching_last = 2657.100084060401
identified_switch = 4395.830549988801


sbs_sum_10D = sbs_sum
vbs_sum_10D = 1641.0806916631
selector_sum_10D = 4234.6070039033
optimal_selection_sum_10D = optimal_selection_sum
optimal_switching_first_10D = optimal_switching_first
optimal_switching_last_10D = optimal_switching_last
identified_switch_10D = identified_switch

sbs_sum_5D = 3386.3677671403
vbs_sum_5D = 475.5923677938
selector_sum_5D = 2080.4475744862
optimal_selection_sum_5D = 1434.2095246840001
optimal_switching_first_5D = 1622.849555104
optimal_switching_last_5D = 1464.4690965342
identified_switch_5D = 2139.0665358946
...
def plot_gap_closed_by_fid(
    ...
    # --- annotate out-of-bounds values in the TOP subplot ---
    ymin_top = _scale_y(-7)   # matches your y-axis lower bound
    ymax_top = 1.1            # matches your y-axis upper bound
...
def plot_gap_closed_bars(
    ...
    # add_row("1", (sbs_sum - optimal_selection_sum) / den, optimal_selection_sum)
    # add_row("2a", (sbs_sum - optimal_switching_first) / den, optimal_switching_first)
    # add_row("2b", (sbs_sum - optimal_switching_last) / den, optimal_switching_last)
    # add_row("3", (sbs_sum - identified_switch) / den, identified_switch)
    add_row("BFGS, B900", 0.0, None)
    add_row("Non-elitist, B0", (sbs_sum - nonelit_sum) / den, nonelit_sum)
...
def plot_selector_dashboard(
    ...
    Three vertically stacked subplots:
      1) Fraction of the gap closed
      2) Algorithm distribution by fid
      3) Switching budget by fid

    - x-axis shared (ticks on all, label only on bottom)
    - Legends: 
        top panel -> bottom-left inside
        middle panel -> in the gap between row1 and row2 (horizontal)
        bottom panel -> bottom-left inside
    """
    pio.kaleido.scope.mathjax = None
    fids = sorted(df["fid"].dropna().astype(int).unique().tolist())

    # ---------- Top subplot: fraction of the gap closed ----------
    methods = ["selector_precision", "static_B60"]
    ...
    for fid, sub in df.groupby("fid", dropna=False):
        ...
        for col in methods:
            ...
            name  = "Dynamic selector" if col == "selector_precision" else "B60"
            if col == "static_B60":
                print(f"DEBUG: fid={fid}, sbs_sum={sbs_sum}, vbs_sum={vbs_sum}, msum={msum}, num={num}, den={den}, score={score}")
    ...
    for method, subm in top_df.groupby("method", sort=False):
        ...
        fig.add_trace(
            go.Scatter(
                x=subm["fid"], y=subm["fraction_scaled"],
    ...
    for _, r in top_df.iterrows():
        x_fid = int(r["fid"])
        y_true = float(r["fraction"])
        y_scaled = float(r["fraction_scaled"])

        if pd.isna(y_scaled):
            continue

        if y_scaled < ymin_top:
            # annotate at lower boundary
            fig.add_annotation(
                x=x_fid, y=ymin_top,
                xref="x1", yref="y1",
                text=f"{y_true:.2f}",
        ...
        elif y_scaled > ymax_top:
            # annotate at upper boundary
            fig.add_annotation(
                x=x_fid, y=ymax_top,
                xref="x1", yref="y1",
                text=f"{y_true:.2f}",
    ...
    fig.add_hline(y=0, line_dash="solid", line_width=2.0, line_color="black", row=1, col=1, layer="below")
    fig.add_hline(y=1, line_dash="dot",   line_width=1.0, line_color="black", row=1, col=1)

    top_tick_orig  = [-7,-6,-5,-4,-3,-2,-1,0,0.5,1.0]
    ...
    for algo in algos_in_data:
        fig.add_trace(
            go.Bar(
                ...
                marker=dict(color=algo_colors[algo]),
                width=0.86, opacity=0.9,
                legendgroup="middle", legend="legend2",
            ),
            row=2, col=1
        )
    fig.update_yaxes(title_text="Proportion of selected algorithms", range=[0,1], row=2, col=1, gridcolor="black")

    # Bottom traces
    fig.add_trace(
        go.Scatter(
            x=fids + fids[::-1], y=upper + lower[::-1],
            fill="toself", fillcolor="rgba(65,105,225,0.35)",
            line=dict(color="rgba(0,0,0,0)"),
            hoverinfo="skip", name="±1σ",
            legendgroup="bottom", legend="legend3",
        ),
        row=3, col=1
    )
    fig.add_trace(
    ...
    fig.update_yaxes(title_text="Switching budget", row=3, col=1, gridcolor="black", zerolinecolor="black", zerolinewidth=1.5,
                     range=[0, 2000])
    ...
    fig.update_xaxes(row=3, col=1, **common_x, title_text="BBOB function", title_standoff=12, showticklabels=True)
    ...
    gap_mid    = (top_bottom + mid_top) / 2.0 - 0.145   # slightly lower to avoid overlap

    # ---- Layout with three legends (no titles)
    fig.update_layout(
        title=dict(text="Selector Behaviour across Functions, 10D", x=0.5, xanchor="center",
        pad=dict(t=0, b=0)),
        width=width, height=height,
        font=dict(family=font_family, size=16, color="black"),
        ...
        # Top legend: bottom-left inside row1
        legend=dict(
            x=0.02, y=top_bottom + 0.03,
        ...
        # Middle legend: centered horizontally
        legend2=dict(
            x=0.5, y=gap_mid,
        ...
        # Bottom legend: bottom-left inside row3
        legend3=dict(
            x=0.02, y=0.31,
...
def sum_min_precisions_at_switch(selector_file: str, algo_file: str) -> float:
    """
    For each (fid, iid, rep) in selector_file, take selector_switch_budget,
    find all rows in algo_file with same (fid, iid, rep, budget),
    take the minimum precision across algorithms, and sum them up.
    """
    # Load CSVs
    df_sel = pd.read_csv(selector_file)
    df_alg = pd.read_csv(algo_file)

    # Merge on fid, iid, rep, and match selector_switch_budget to budget
    merged = df_sel.merge(
        df_alg,
        left_on=["fid", "iid", "rep", "selector_switch_budget"],
        right_on=["fid", "iid", "rep", "budget"],
        how="inner"
    )

    print(merged)

    # For each (fid, iid, rep), get the minimum precision
    min_precisions = merged.groupby(["fid", "iid", "rep"])["precision"].min()

    # Return the sum
    return float(min_precisions.sum())

import pandas as pd

def sum_static_at_min_precision_budget(
    selector_file: str,
    algo_file: str,
    tie_breaker: str = "first",  # "first" (smallest budget) or "last" (largest budget) on ties
) -> float:
    """
    For each (fid,iid,rep), find the budget where the minimum precision occurs in file2.
    Then extract static_B{budget} from file1 at the same (fid,iid,rep).
    Return the sum over all rows.

    Parameters
    ----------
    file1 : str
        Path to the first CSV file (with static_B* columns).
    file2 : str
        Path to the second CSV file (with algorithm precisions).
    tie_breaker : {"first", "last"}, optional
        How to resolve ties if multiple budgets have the same min precision.

    Returns
    -------
    float
        Sum of the extracted static_B values.
    """
    df1 = pd.read_csv(selector_file)
    df2 = pd.read_csv(algo_file)

    results = []
    for (fid, iid, rep), group in df2.groupby(["fid", "iid", "rep"]):
        # find min precision
        min_val = group["precision"].min()
        min_budgets = group.loc[group["precision"] == min_val, "budget"].tolist()

        if tie_breaker == "first":
            chosen_budget = min_budgets[0]
        elif tie_breaker == "last":
            chosen_budget = min_budgets[-1]
        else:
            raise ValueError("tie_breaker must be 'first' or 'last'")

        # extract from df1
        row = df1[(df1["fid"] == fid) & (df1["iid"] == iid) & (df1["rep"] == rep)]
        if not row.empty:
            col = f"static_B{chosen_budget}"
            if col in row.columns:
                results.append(row.iloc[0][col])

    return sum(results)

import pandas as pd

def sum_static_at_fid_min_budget(selector_file: str, fid_budget_file: str) -> float:
    """
    For each fid in fid_budget_file:
      - find the *lowest* best_budget for that fid,
      - in selector_file sum the values of column static_B{best_budget} over all rows with that fid.
    Return the sum across all fids.

    Assumes selector_file has columns like static_B20, static_B40, ... and columns fid,iid,rep,...
    """
    df_sel = pd.read_csv(selector_file)
    df_fb  = pd.read_csv(fid_budget_file)

    # lowest best_budget per fid
    fid_min = df_fb.groupby("fid", as_index=False)["best_budget"].min()

    total = 0.0
    for _, row in fid_min.iterrows():
        fid = row["fid"]
        budget = int(row["best_budget"])
        col = f"static_B{budget}"
        # sum the column for that fid, then add to total
        total += df_sel.loc[df_sel["fid"] == fid, col].sum()

    return float(total)

def plot_gap_closed_bars_two_dims_from_sums(
    *,
    # --- 10D inputs
    sbs_sum_10D: float,
    vbs_sum_10D: float,
    scenario_sums_10D: dict[str, float],
    # --- 5D inputs
    sbs_sum_5D: float,
    vbs_sum_5D: float,
    scenario_sums_5D: dict[str, float],
    # --- styling / IO
    title: str = "Fraction of SBS–VBS gap closed (10D vs 5D)",
    font_family: str = "Latin Modern Roman",
    width: int = 880,
    height: int = 480,
    color_10D: str = "#ff7f0e",   # per request
    color_5D: str = "royalblue",  # per request
    save_pdf: str | None = None,
):
    """
    One bar plot with two groups:
      - Left: 10D scenarios (sorted desc by fraction closed), colored color_10D
      - Right: 5D scenarios (sorted desc), colored color_5D
    Returns (fig, res_df).
    """
    pio.kaleido.scope.mathjax = None

    def rows_from(sbs_sum, vbs_sum, scenarios: dict[str, float], dim_label: str):
        den = sbs_sum - vbs_sum
        if den == 0:
            raise ValueError(f"Denominator (sbs_sum - vbs_sum) is zero for {dim_label}.")
        rows = []
        for name, raw in scenarios.items():
            score = (sbs_sum - float(raw)) / den
            rows.append({
                "dimension": dim_label,
                "x": f"{dim_label} – {name}",
                "scenario": name,
        ...
        # sort desc within dimension
        return sorted(rows, key=lambda r: r["score"], reverse=True)

    rows_10 = rows_from(sbs_sum_10D, vbs_sum_10D, scenario_sums_10D, "10D")
    rows_5  = rows_from(sbs_sum_5D,  vbs_sum_5D,  scenario_sums_5D,  "5D")

    # concat with 10D first (left), then 5D (right)
    rows_all = rows_10 + rows_5
    xs = [r["x"] for r in rows_all]
    ys = [r["score"] for r in rows_all]
    colors = [color_10D]*len(rows_10) + [color_5D]*len(rows_5)
    ...
    # vertical separators between groups (optional; comment out if not desired)
    # Place a light separator at the boundary between 10D and 5D bars
    if rows_10 and rows_5:
        fig.add_vline(
            x=len(rows_10)-0.5,  # between last 10D and first 5D
            line_width=1,
            line_dash="dot",
            line_color="black",
        )
    ...
    res = pd.DataFrame(rows_all, columns=["dimension", "scenario", "x", "score", "raw_sum", "sbs_sum", "vbs_sum"])
    return fig, res

if __name__ == "__main__":
    df_sel = pd.read_csv("data/selector_old_tuning_200_200_with_sbs.csv")
    # df_alg = pd.read_csv("data/precision_files/A2_precisions_newInstances_10D.csv")
    # df_budgets = pd.read_csv("data/best_static_budget_per_fid.csv")
    # print(df_sel["selector_precision"].sum())
    # scenario_sums_10D = {
    # "Optimal selection": optimal_selection_sum_10D,
    # "Optimal switch (first)": optimal_switching_first_10D,
    # "Optimal switch (last)":  optimal_switching_last_10D,
    # "Identified switch":      identified_switch_10D,
    # "Dynamic selector": selector_sum_10D
    # }
    # scenario_sums_5D = {
    #     "Optimal selection": optimal_selection_sum_5D,
    #     "Optimal switch (first)": optimal_switching_first_5D,
    #     "Optimal switch (last)":  optimal_switching_last_5D,
    #     "Identified switch":      identified_switch_5D,
    #     "Dynamic selector": selector_sum_5D
    # }

    # fig_both, res_both = plot_gap_closed_bars_two_dims_from_sums(
    #     sbs_sum_10D=sbs_sum_10D,
    #     vbs_sum_10D=vbs_sum_10D,
    #     scenario_sums_10D=scenario_sums_10D,
    #     sbs_sum_5D=sbs_sum_5D,
    #     vbs_sum_5D=vbs_sum_5D,
    #     scenario_sums_5D=scenario_sums_5D,
    #     title="Ablation Scenarios: 10D vs 5D",
    #     save_pdf="ablation_bars_10D_5D.pdf",
    # )

    # print(res_both)

    plot_selector_dashboard(
        df=df_sel,
        df_algos=pd.read_csv("data/selector_old_tuning_200_200_with_algos.csv"),
        save_pdf="selector_dashboard_10D_test.pdf",
        width=980,
        height=1400,
    )