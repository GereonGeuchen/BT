...
data_dir = "A1_data_ela_globally_normalized" # Add _normalized here if you want to use the normalized data
...
def preprocess(df, target_col, use_excluded_columns=True, budget=None):
    if use_excluded_columns:
        X = df.drop(columns=excluded_columns + ["fid", "iid", "high_level_category", "rep"], errors='ignore')
         ...
         # Identify columns that contain inf, -inf or NaN
        inf_columns = df.columns[(df == np.inf).any() | (df == -np.inf).any() | df.isna().any()]
        
        # Identify columns ending with ".costs_runtime"
        runtime_columns = [col for col in df.columns if col.endswith(".costs_runtime")]
        
        # Combine all columns to drop
        drop_columns = set(inf_columns.tolist() + runtime_columns)
        
        if drop_columns:
            print(f"Columns dropped for budget {budget}: {sorted(drop_columns)}")
...
if __name__ == "__main__":
    initial_sampling_rf()
    rf_over_budgets(include_variance_band=True)
