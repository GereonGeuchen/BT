...
def plot_switching_points_per_rep(df, fid, iid, min_budget=0, save_plot=False):
    ...
    if save_plot:
        dir_path = f"../results/precisions/switching_points_per_rep_with_algos_every50"
        os.makedirs(dir_path, exist_ok=True)
        out_path = os.path.join(dir_path, f"fid_{fid}_iid_{iid}_minb_{min_budget}.pdf")
...
def plot_best_budget_percentages(df, budget_range=range(50, 1001, 50), min_budget=0):
    ...
    plt.title(f"Percentage of runs with each budget as optimal switching point", fontsize=16)
...
def plot_precision_gradient_per_rep(df, fid, iid, min_budget=0, save_plot=False):
    ...
    if save_plot:
        dir_path = f"../results/precisions/precisions_gradient_50"
        os.makedirs(dir_path, exist_ok=True)
...
if __name__ == "__main__":
    ...
    plot_best_budget_percentages(df, budget_range=range(50, 1001, 50), min_budget=0)
    # for fid in range(1, 25):
    #     for iid in range(1, 6):
    #         plot_switching_points_per_rep(df, fid, iid, min_budget=0, save_plot=True)
