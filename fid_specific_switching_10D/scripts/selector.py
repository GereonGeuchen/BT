...
class SwitchingSelector:
    ...
    def simulate_single_run(self, fid, iid, rep, ela_dir="../data/ela_with_state_test_data", precision_file="../data/A2_precisions_test.csv", budgets=range(50, 1001, 50)):
        ...
        for budget in budgets:
            ela_path = Path(ela_dir) / f"A1_B{budget}_10D_ela_with_state.csv"
        ...
        for fid in fids:
            for iid in iids:
                for rep in reps:
                    ...
                    row["selector_switch_budget"] = result["switch_budget"] or 2000

                    # Static switchers
                    for b in budgets:
                        col_name = f"static_B{b}"
                        if b < 2000:
                            ela_path = Path(ela_dir) / f"A1_B{b}_10D_ela_with_state.csv"
                            if not ela_path.exists():
                                row[col_name] = None
                                continue
...
if __name__ == "__main__":
    selector = SwitchingSelector(
        selector_model_dir="../data/models/tuned_models/switching_models_10D",
        performance_model_dir="../data/models/trained_models/algo_performance_models_trained_normalized_log10_200_10D"
    )
    selector.evaluate_selector_to_csv(
        fids=list(range(1, 25)),
        iids=[6, 7],
        reps=list(range(20)),
        save_path="../results/newInstances_normalized_log10_10D/selector_old_tuning_200_200.csv",
        ela_dir="../data/ela_for_testing/A1_data_ela_cma_std_normalized_10D_newInstances",
        precision_file="../data/precision_files/A2_precisions_newInstances_10D.csv"
    )
